...
public class ImpuestoCTR extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Gson gson = new Gson();
        ImpuestoDTO impuestoDTO;
        ImpuestoDAO impuestoDAO;
        impuestoDTO = gson.fromJson(cadenaJSON, ImpuestoDTO.class);
        impuestoDAO = new ImpuestoDAO();

        switch (impuestoDTO.getBandera()) {
            case 1:
                if (impuestoDAO.agregar(impuestoDTO)) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (impuestoDAO.eliminar(impuestoDTO)) {
                    //Operación exitosa => al cliente
                } else {
                    //Operación errónea => al cliente
                }
            ...
            case 4:
//                PrintWriter out = response.getWriter();
                String json = gson.toJson(impuestoDAO.seleccionarSegunId(impuestoDTO));
                if (json != null) {
            ...
            case 5:
                //Cargar Tabla
                response.setContentType("application/json, charset=UTF-8");
                String cadenaImpuesto = gson.toJson(impuestoDAO.seleccionarTodos());
