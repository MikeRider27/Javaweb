...
public class PerfilCTR extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        PerfilDTO perfilDTO;
        PerfilDAO perfilDAO;
        perfilDTO = gson.fromJson(cadenaJSON, PerfilDTO.class);
        perfilDAO = new PerfilDAO();

        switch (perfilDTO.getBandera()) {
            case 1:
                if (perfilDAO.agregar(perfilDTO)) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (perfilDAO.eliminar(perfilDTO)) {
                    //Operación exitosa => al cliente
                } else {
                    //Operación errónea => al cliente
                }
            ...
            case 4:
//                PrintWriter out = response.getWriter();
                String json = gson.toJson(perfilDAO.seleccionarSegunId(perfilDTO));
                if (json != null) {
                ...
                //Cargar Tabla
                response.setContentType("application/json, charset=UTF-8");
                String cadenaPerfil = gson.toJson(perfilDAO.seleccionarTodos());
                if (cadenaPerfil != null) {
                    //enviar al js la cadena 
                    System.out.println("Cadena " + cadenaPerfil);
                    out.println(cadenaPerfil);
                } else {
