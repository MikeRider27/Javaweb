...
public class NacionalidadCTR extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        nacionalidadDTO = gson.fromJson(cadenaJSON, NacionalidadDTO.class);
        nacionalidadDAO = new NacionalidadDAO();

        switch (nacionalidadDTO.getBandera()) {
            case 1:
                if (nacionalidadDAO.agregar(nacionalidadDTO)) {
                ...
                break;
            case 2:
                if (nacionalidadDAO.modificar(nacionalidadDTO)) {
                    //Operación exitosa => al cliente
                } else {
                    //Operación errónea => al cliente
                }
                break;
            case 3:
                if (nacionalidadDAO.eliminar(nacionalidadDTO)) {
                    //Operación exitosa => al cliente
                } else {
                    //Operación errónea => al cliente
                }
            ...
            case 4:
//                PrintWriter out = response.getWriter();
                String json = gson.toJson(nacionalidadDAO.seleccionarSegunId(nacionalidadDTO));
                if (json != null) {
