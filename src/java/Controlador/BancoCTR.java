...
public class BancoCTR extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        BancoDTO bancoDTO;
        BancoDAO bancoDAO;
        bancoDTO = gson.fromJson(cadenaJSON, BancoDTO.class);
        bancoDAO = new BancoDAO();

        switch (bancoDTO.getBandera()) {
            case 1:
                if (bancoDAO.agregar(bancoDTO)) {
                ...
                break;
            case 2:
                if (bancoDAO.modificar(bancoDTO)) {
                    //Operación exitosa => al cliente
                } else {
                    //Operación errónea => al cliente
                }
            ...
            case 4:
//                PrintWriter out = response.getWriter();
                String json = gson.toJson(bancoDAO.seleccionarSegunId(bancoDTO));
                if (json != null) {
            ...
            case 5:
                //Cargar Tabla
                response.setContentType("application/json, charset=UTF-8");
                String cadenaBancosTodos = gson.toJson(bancoDAO.seleccionarTodos());
                if (cadenaBancosTodos != null) {
                    //enviar al js la cadena 
                    System.out.println("Cadena " + cadenaBancosTodos);
                    out.println(cadenaBancosTodos);
                } else {
...
//                EmpleadoDAO empleadoDAO = new EmpleadoDAO();
                String cadenaProveedor = gson.toJson(bancoDAO.seleccionarTodos());
